...
{
    public partial class SalesContractList : UserControl
    {
        ...
        /// </summary>
        public static string SalesContractNum
        {
            get
            {
                return contractNum;
...
";
        ...
        private void SetAttributes()
        {
            ...
            toolTip1.SetToolTip(txtLookup, String.Format(oDict.GetWord("look_for_targets"), "; ", "Contract Number", "Customer Code and Customer Name"));
            toolTip1.SetToolTip(cmdLookup, String.Format(oDict.GetWord("look_for_targets"), "; ", "Contract Number", "Customer Code and Customer Name"));
            ...
            colItemNumber.Text = oDict.GetWord("sales_contract_num");
        ...
        private void SetLvwList()
        {
            ...
            lvwList.Tag = new Guid("7D4E1598-CC02-4c06-A920-F6FF0E5191C8");
        ...
        private void ShowRecord()
        {
            ...
            if (lvwList.SelectedItem != null)
            {
                ...
                SalesContractRecord contract = new SalesContractRecord();
                contract.SalesContractId = itemId;
                contract.EditMode = Common.Enums.EditMode.Edit;
                contract.Show();

                contract.Closed += new EventHandler(contract_Closed);
            }
        }
        ...
        private void BindList()
        {
            ...
            DataSet ds = ViewService.Default.GetSalesContractList(whereClause, orderBy);
            DataTable dt = ds.Tables[0];

            foreach (DataRow row in dt.Rows)
            {
                Guid orderId = (Guid)row["OrderSCId"];

                ListViewItem objItem = this.lvwList.Items.Add(row["SCNumber"].ToString());  // Number
                #region Icon
                ...
                #endregion
                #region Attachment
                if (xPort5.Controls.Utility.Resources.HasAttachment(row["SCNumber"].ToString())) // Attachment
                {
                ...
                objItem.SubItems.Add(row["SCDate"] != DBNull.Value ? Convert.ToDateTime(row["SCDate"]).ToString("yyyy-MM-dd") : "");              // SC Date
        ...
        private void DoLookup()
        {
            ...
            if (!(String.IsNullOrEmpty(target)))
            {
                ...
                _CurSqlWhere = _BaseSqlWhere +
                               String.Format(" AND ( ([SCNumber] LIKE '%{0}%') OR ([CustomerCode] LIKE '%{0}%') OR ([CustomerName] LIKE N'%{0}%') OR ([CustomerName_Chs] LIKE '%{0}%') OR ([CustomerName_Cht] LIKE '%{0}%') )", target);
                BindList();
                this.Update();
            }
        }
            ...
            if (selectedIndex >= 0)
            {
                switch (selectedIndex)
                {
                    case 1:         // 7 dyas
                        ...
                        _CurSqlWhere = _BaseSqlWhere + String.Format(" AND (CONVERT(NVARCHAR(10), [SCDate], 20) >= '{0}')", filter) + String.Format(" AND (CONVERT(NVARCHAR(10), [SCDate], 20) <= '{0}')", today);
                        break;
                    case 2:         // 30 days
        ...
        private void lvwList_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (lvwList.SelectedItem != null)
            {
                ...
                contractNum = this.lvwList.SelectedItem.Text;
            }
        }
        ...
        void contract_Closed(object sender, EventArgs e)
        {
            this.ansList.Buttons[10].Enabled = true;
            this.ansList.Buttons[12].Enabled = true;
        }
        ...
        private void ShowAttachmentManager()
        {
            if (lvwList.SelectedItem != null)
            {
                string scNumber = this.lvwList.SelectedItem.Text;
                Guid itemId = new Guid(lvwList.SelectedItem.SubItems[1].Text);

                AttachmentManager attachWizard = new AttachmentManager();
                attachWizard.OrderId = itemId;
                attachWizard.OrderNumber = scNumber;
